<template>
    <div class="channels__container">
        <h2 class="ui inverted center aligned header">Channels <i class="add square icon add_channel" @click.prevent="openChannelModal"></i></h2>
    ...
    methods: {
      addListeners () {
        this.channelsRef.on('child_added', snap => {
          this.channels.push(snap.val()) // { id: id_aleatoire, name: name }
          ...
          // Rajout du listeners pour gérer les notifs
      ...
      addChannel () {
        ...
        // this.channelsRef.child(key) => root/channels/id_aleatoire/id ou name
      ...
      detachListeners () {
        ...
        this.channels.forEach(el => {
          this.messagesRef.child(el.id).off() // messages/fjjoernvjdnj
        })
      },
